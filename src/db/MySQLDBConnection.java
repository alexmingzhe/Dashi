...
// set up connection between yelp.api to database
public class MySQLDBConnection implements DBConnection {
	...
	public JSONArray recommendRestaurants(String userId) {
		try {
			...
			// if use hashmap, could see the frequency of visited history, used
			// to sort
			// to get all the categories the user has visited
			...
			for (String businessId : allRestaurants) {
				// Perform filtering, recommend new restaurant that didn't visit
	...
	public JSONObject getRestaurantsById(String businessId, boolean isVisited) {
		try {
			...
			while (rs.next()) {
				...
				// transform the Restaurant.class to JSONObject.class
	...
	public Set<String> getCategories(String businessId) {
		...
		try {
			...
			statement.setString(1, businessId);
			ResultSet rs = statement.executeQuery();
			// business id is unique, "if"loop is enough
			if (rs.next()) {
				String[] categories = rs.getString("categories").split(",");
				for (String category : categories) {
					visitedCategories.add(category.trim());
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public JSONArray searchRestaurants(String userId, double lat, double lon) {
		/*
		 * 1.Fetch restaurant from Yelp API 2.save in database 3.return json
		 * Array with filtered results
		 */
		try {
			// Connect to Yelp API
