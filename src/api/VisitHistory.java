...
public class VisitHistory extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			/* 
			 * 1. use a Set<String> to getVisitedResataurants from given userId
			 * 2. iterate the Set and use function to get every single visited_business_id into a JSONArray
			 * 3. use the Rpcparser.writeoutput to send array to frontend?
			 */
			if (request.getParameterMap().containsKey("user_id")) {
				String userId = request.getParameter("user_id");
				Set<String> visited_business_ids = connection.getVisitedRestaurants(userId);
				for (String visited_business_id: visited_business_ids) {	
					array.put(connection.getRestaurantsById(visited_business_id,true));
				}
